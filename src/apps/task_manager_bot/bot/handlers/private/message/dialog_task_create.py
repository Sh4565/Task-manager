...
from apps.task_manager_bot.bot.filters.private import PrivateFilter
...
@dialog_task_router.message(PrivateFilter(), CreateTask.title)
    ...
    try:
        ...
        if data['description']:
            await message.answer(
                ...
                reply_markup=callback_data.reply_edit_description_keyboard(
                    date=data['date'],
                    data=data,
        ...
        await message.answer(
            text=text,
            reply_markup=callback_data.reply_return_add_task_keyboard(
                date=data['date'],
                language=language_user
            )
        )
...
@dialog_task_router.message(PrivateFilter(), CreateTask.description)
async def create_description_task(message: Message, state: FSMContext,
...
@dialog_task_router.message(PrivateFilter(), CreateTask.time)
async def create_time_task(message: Message, state: FSMContext, language_user: str) -> None:
    try:
